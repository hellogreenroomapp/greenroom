<template>
  <div class="relative">
    <button
      class="w-full px-3 py-2 text-sm bg-bg border border-border rounded-md text-text focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 flex items-center justify-between"
      @click.stop="toggleDropdown"
    >
      <span class="flex items-center">
        <span v-if="selectedBrand?.logoUrl" class="mr-2">
    ...
    <Transition name="dropdown">
      <div
        ...
        class="absolute z-[100] w-full mt-1 bg-card border border-border rounded-md shadow-xl flex flex-col max-h-80"
        style="top: 100%;"
      >
        <div class="overflow-y-auto flex-1 py-1 min-h-0">
          ...
          <!-- Empty State -->
          <div v-if="ownedBrands.length === 0 && invitedBrands.length === 0" class="px-3 py-4 text-center">
            <p class="text-sm text-muted mb-3">No brands yet</p>
          </div>
        </div>

        <!-- Divider -->
        <div v-if="(ownedBrands.length > 0 || invitedBrands.length > 0)" class="border-t border-border flex-shrink-0"></div>

        <!-- Create Brand Button - Always visible at bottom -->
        <div class="px-3 py-2 bg-card flex-shrink-0">
          <button
...
onMounted(async () => {
  if (authStore.userId && brands.value.length === 0) {
